    ...
    <html>
      <head>
        ...
        <style>
          ...
          .share-link { color: #2980b9; text-decoration: underline; cursor: pointer; display: block; margin-bottom: 10px; text-align: center; }
          .print-btn { background-color: #2980b9; color: white; border: none; padding: 10px 20px; font-size: 1em; cursor: pointer; border-radius: 12px; width: 100%; margin-top: 20px; }
          @media print {
            body, .share-link, .print-btn { display: none; }
            #resume-content { margin: 0; }
          }
        </style>
      </head>
      <body>
        <div style="max-width: 650px; margin: auto; padding: 35px; background-color: #ffffff; border-radius: 12px; box-shadow: 0 8px 20px rgba(0, 0, 0, 0.15);" id="resume-content">
          ${generated}
        </div>
        <div style="max-width: 650px; margin: auto; margin-top: 20px">
          <a class="share-link" onclick="copyLink()">Click here to copy shareable link</a>
          <button class="print-btn" onclick="printResume()">Print Resume</button>
        </div>
        <script>
          function copyLink() {
            const link = "https://shareable-resume-builder-2024.netlify.app/";
